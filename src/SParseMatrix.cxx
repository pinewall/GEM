...
SparseMatrix::SparseMatrix (UINT _row_dim, UINT _col_dim)
{
    row_dim = _row_dim;
    col_dim = _col_dim;
    row_ptr = new UINT [row_dim + 1];
    memset (row_ptr, 0, sizeof (UINT) * (row_dim + 1));
    //col_ptr = new UINT [col_dim + 1];
    ...
    row_list = new UINT [buffer_size];
    col_list = new UINT [buffer_size];
    value_list = new double [buffer_size];
    assert ((row_list != (UINT *) 0));
    assert ((col_list != (UINT *) 0));
    assert ((value_list != (double *) 0));
}

SparseMatrix::SparseMatrix (UINT _row_dim, UINT _col_dim, UINT _current_size, UINT * _row_list, UINT * _col_list, double * _value_list)
{
    row_dim = _row_dim;
    col_dim = _col_dim;
    row_ptr = new UINT [row_dim + 1];
    ...
    memcpy (row_list, _row_list, sizeof (UINT) * buffer_size);
    memcpy (col_list, _col_list, sizeof (UINT) * buffer_size);
...
SparseMatrix::SparseMatrix (SparseMatrix * copyer)
{
    ...
    col_list = new UINT [buffer_size];
    value_list = new double [buffer_size];
    memcpy (row_ptr, copyer->row_ptr, (row_dim + 1) * sizeof (UINT));
    memcpy (row_list, copyer->row_list, current_size * sizeof (UINT));
    memcpy (col_list, copyer->col_list, current_size * sizeof (UINT));
...
void SparseMatrix::Add_entry (UINT row, UINT col, double value)
{
...
void SparseMatrix::Resize ()
{
    assert (increase > 0);
    buffer_size += increase;
    UINT * tmp_row_list = new UINT [buffer_size];
    UINT * tmp_col_list = new UINT [buffer_size];
    double * tmp_value_list = new double [buffer_size];
    memcpy (tmp_row_list, row_list, sizeof (UINT) * current_size);
    memcpy (tmp_col_list, col_list, sizeof (UINT) * current_size);
...
{
    UINT * tmp_row_list = new UINT [current_size];
    UINT * tmp_col_list = new UINT [current_size];
...
#endif
    UINT * new_row_list = new UINT [current_size];
    UINT * new_col_list = new UINT [current_size];
    UINT * new_row_ptr = new UINT [col_dim + 1];
    double * new_value_list = new double [current_size];

    // calculate number of nonzero element in each column
    UINT * temp = new UINT [col_dim];
    //memset (temp, 0, sizeof (UINT) * col_dim);
...
SparseMatrix * SparseMatrix::Matrix_add (SparseMatrix * adder)
{
    ...
    UINT id1, id2;                       // record current location
    UINT id1_max, id2_max;
...
void SparseMatrix::Matrix_vector_multiple (double * output, UINT osize, double * input, UINT isize)
{
#ifdef SPARSEMATRIX_DEBUG
